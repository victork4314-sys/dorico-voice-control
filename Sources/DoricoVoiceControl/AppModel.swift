...
final class AppModel: ObservableObject {
    ...
    private let safetyPolicy = VoiceSafetyPolicy()
    ...
    private func wireSpeechController() {
        speechController.onPartialTranscript = { [weak self] value in self?.partialTranscript = value }
        speechController.onFinalTranscript = { [weak self] value in self?.handleFinalTranscript(value) }
        ...
        speechController.onListeningChanged = { [weak self] value in self?.isListening = value }
        speechController.onAudioFormatChanged = { [weak self] value in self?.audioFormat = value }
    }
    ...
    private func handleFinalTranscript(_ transcript: String) {
        ...
        let batch = DoricoVoiceLanguage.parseBatch(transcript, aliases: preferences.aliases, calibration: preferences.calibration)
        currentBatch = batch
        do {
    ...
    private func persistPreferences() {
        ...
        catch { log("Storage error", error.localizedDescription) }
    }
