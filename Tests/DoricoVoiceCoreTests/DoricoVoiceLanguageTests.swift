...
final class DoricoVoiceLanguageTests: XCTestCase {
    ...
    func testBarRepeatIsBounded() {
        XCTAssertEqual(DoricoVoiceLanguage.parseBatch("move right sixty four bars").commands.first?.label, "Move right 64 bars")
    ...
    func testGenericDoricoCommand() {
        XCTAssertEqual(DoricoVoiceLanguage.parseBatch("Dorico command note input options").commands.first?.label, "Dorico command: note input options")
    }
